...
void ResultReport::toCSV(std::string file) {
	...
	if (results[0].isField) {
		...
		for (auto r : results) {
			 myfile<<r.txId<<","<<r.rxId<<","<<r.Ex.x<<","<<r.Ex.y<<dot(r.Ex,r.Ex)<<","<<r.Ey.x<<","<<r.E.y<<","<<dot(r.Ey,r.Ey)<<","<<r.Ez.x<<","<<r.Ex.y<<","<<dot(r.Ez,r.Ez)<<","<<r.position.x<<","<<r.position.y<<","<<r.position.z<<","<<r.raysHit<<","<<r.radius<<","<<r.d<<","<<r.origin.x<<","<<r.origin.y<<","<<r.origin.z<< std::endl;
